...
**Status:** **COMPLETE** (2026-09-11) on `wip_phase8` for the acceptance
criteria below, including the actuator-noise amendment. Non-zero
`ResidualCorrector`, bounded Cartesian→Δq mapping, SafetyProjector clamping,
nominal/stop fallback, sim-only offline training, residual-on vs residual-off
reports, and a configuration-driven joint actuator noise model are implemented.
Live Kit `rsl_rl` loops remain optional host wrappers around the same contracts;
...
6. **Actuator noise model:** provide a configuration-driven joint-space
   actuator disturbance model used in residual training and residual-on/off
   evaluation so corrections are exercised under motor/servo execution noise,
   not only constant tip bias or measured-joint observation noise. Distinguish
   clearly from:
   - tip bias / TCP model mismatch (systematic Cartesian offset), and
   - measurement noise on sensed joint positions.
   Enabling or changing the model requires retraining residual checkpoints and
   re-running the Phase 8 residual GUI smoke
...
- Residual train/eval paths can enable a documented actuator noise model
  (`config/actuator_noise.yml`); enabling or changing that model invalidates
